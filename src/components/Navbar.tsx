...
class NavbarComponent extends React.Component<NavbarProps, NavbarState> {
	...
	createFile = (e: any) => {
		...
		if (e.target.files[0].type !== "text/plain") {
			this!.fileInput!.current!.value = "";
			this.setState({ modalWrongFormat: true, fileValue: null, fileName: null })
			return;
		}
		...
		reader.onload = e => {
			...
			this.setState({fileName: name, fileValue: value}, () => {
				...
				if(!fileExist) {
					...
					// Creating new file
					this.props.store!.fileStore.createFile(this.state!.fileName!, this.state!.fileValue!);
					return;
				}
	...
	render() {
		...
		return (
			<Navbar collapseOnSelect expand="lg" bg="dark" variant="dark">
				...
				<Modal
					isOpen={this.state.modalWarningOpen}
					style={this.getModalStyle()}>
					<h5>Файл з таким ім'ям вже збережено в програмі. Ви можете перезаписати файл чи скористатися існуючим</h5>
					<Button onClick={() => this.continueNew()}>Замінити та продовжити</Button>
					<Button onClick={() => this.continueOld()}>Продовжити роботу з існуючим</Button>
				</Modal>

				<Modal
					isOpen={this.state.modalWrongFormat}
					style={this.getModalStyle()}>
					<h5>Невірний формат файлу</h5>
					<Button variant="danger" onClick={() => this.setState({ modalWrongFormat: false })}>Закрити</Button>
				</Modal>
				...
				<Navbar.Collapse id="responsive-navbar-nav">
					<Nav className="mr-auto">
						...
						<NavDropdown title="Робота за файлом" id="collasible-nav-dropdown">
							...
							<NavDropdown.Divider />
							<NavDropdown.Item href="#action">Зберегти як .txt</NavDropdown.Item>
							<NavDropdown.Item href="#action">Зберегти як .cvc</NavDropdown.Item>
